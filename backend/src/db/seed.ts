...
async function seed() {
  ...
  const hallLayout = { rows: ["A", "B", "C", "D", "E"], seatsPerRow: 10 };
  const hallResult = await pool.query(
    `INSERT INTO halls (name, total_seats, layout_json)
     VALUES ($1, $2, $3)
     RETURNING id`,
    ["Aurora Hall", hallLayout.rows.length * hallLayout.seatsPerRow, hallLayout]
  );

  const hallId = hallResult.rows[0].id as string;

  const seatValues: Array<[string, string, number, string]> = [];
  for (const rowLabel of hallLayout.rows) {
    for (let seatNumber = 1; seatNumber <= hallLayout.seatsPerRow; seatNumber += 1) {
